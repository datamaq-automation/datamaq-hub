"""Gateway for querying YPF career and talent opportunities."""
...
DEFAULT_YPF_CAREERS_URL = "https://career8.successfactors.com/career"
DEFAULT_USER_AGENT = (
...
class YpfTalentGateway(PortalEmpleoPort):
    """Implementación del puerto PortalEmpleoPort para el portal de talento de YPF."""

    def __init__(
        self,
        base_url: str = DEFAULT_YPF_CAREERS_URL,
    ...
    def buscar_ofertas(
        ...
        """Consulta ofertas en el portal de YPF y las mapea al dominio."""
        headers = {
        ...
        try:
            ...
            # Resiliencia: si el servicio de YPF está caído o bloquea, retornamos lista vacía
            return []
    ...
    def _parsear_resultados(self, data: dict[str, Any]) -> list[OfertaLaboral]:
        ...
        # Estructuras comunes en SuccessFactors / OData: {"d": {"results": [...]}} o lista directa
        items: list[Any] = []
        ...
        for item in items:
            ...
            ubicacion = str(item.get("location") or item.get("city") or "Argentina")
            descripcion = str(
            ...
            oferta = OfertaLaboral(
                id_oferta=f"ypf-{req_id}" if req_id else f"ypf-{hash(titulo)}",
                titulo=titulo,
                empresa="YPF S.A.",
                ubicacion=ubicacion,

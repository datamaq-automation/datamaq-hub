...
class HorarioBloqueDTO(BaseModel):
    ...
    dia: str = Field(
        description="Día de la semana: LUNES, MARTES, MIERCOLES, JUEVES, VIERNES, SABADO"
    )
...
class CargoDocenteDTO(BaseModel):
    ...
    id_cargo: str = Field(
        description="Identificador del cargo o secuencia (ej. 'CARGO-01')"
    )
    establecimiento: str = Field(
        description="Nombre de la escuela o institución (ej. 'EEST N° 1 Pilar')"
    )
    distrito: str = Field(
        default="", description="Distrito escolar (ej. 'Pilar', 'Tigre')"
    )
    cargo_asignatura: str = Field(
        description="Nombre de la materia o cargo (ej. 'Electrotecnia 4to')"
    )
    revista: str = Field(
        default="TITULAR",
    ...
    ige: str = Field(
        default="", description="Identificador de Gestión Educativa (DGCyE PBA)"
    )
    modulos: int = Field(
        default=0, ge=0, description="Cantidad de módulos o horas cátedra semanales"
    )
    es_cargo_base: bool = Field(
        default=False,
...
class RegistrarDesignacionInputDTO(BaseModel):
    ...
    docente_cuit: str = Field(
        description="CUIT del docente titular/suplente (ej. '20-36528392-4')"
    )
    ige: str = Field(
        default="", description="Identificador de Gestión Educativa oficial (DGCyE PBA)"
    )
    establecimiento: str = Field(
        description="Nombre del establecimiento educativo (ej. 'EEST N° 1 Pilar')"
    )
    distrito: str = Field(
        default="", description="Distrito escolar (ej. 'Pilar', 'Tigre')"
    )
    cargo_asignatura: str = Field(
        description="Asignatura o cargo (ej. 'Electrotecnia 4to')"
    )
    revista: str = Field(
        default="TITULAR",
        description="Situación de revista: TITULAR, PROVISIONAL, SUPLENTE",
    ...
    fecha_desde: str = Field(
        description="Fecha de inicio/toma de posesión en formato YYYY-MM-DD"
    )
...
class CesarDesignacionInputDTO(BaseModel):
    ...
    fecha_hasta: str = Field(
        description="Fecha efectiva del cese en formato YYYY-MM-DD"
    )
...
class ConflictoDTO(BaseModel):
    ...
    tipo: str = Field(
        description="Tipo de conflicto: SUPERPOSICION_HORARIA, TRASLADO_INSUFICIENTE, etc."
    )
    severidad: str = Field(
        description="Severidad: CRITICO (incompatible) o ADVERTENCIA"
    )
    dia: str | None = Field(default=None, description="Día donde ocurre el conflicto")
    cargos_involucrados: list[str] = Field(
        default_factory=list[str], description="IDs de cargos afectados"
    )
    descripcion: str = Field(description="Explicación detallada del conflicto")
    minutos_solapamiento_o_traslado: int = Field(
        default=0,
        description="Minutos de superposición o minutos disponibles para el viaje",
    )
...
class ResultadoCompatibilidadDTO(BaseModel):
    ...
    es_compatible: bool = Field(
        description="True si no existen superposiciones horarias críticas"
    )
    ...
    total_minutos_semanales: int = Field(
        description="Total de minutos semanales frente a alumnos"
    )
